...
onMounted(async () => {
  try {
    const response = await fetch(`http://localhost:3000/api/prace/${workId}`)
      ...
      <div v-else-if="praca">
        ...
        <div class="work-card">
          ...
          <div class="card-grid">
            ...
            <div v-if="visibleFields.data_rozpoczecia" class="card-item"><strong>Data rozpoczęcia:</strong> <span>{{ praca.data_rozpoczecia }}</span></div>
            <div v-if="visibleFields.data_zakonczenia" class="card-item"><strong>Data zakończenia:</strong> <span>{{ praca.data_zakonczenia }}</span></div>
...
<style scoped>
/* ZMIANA: Generalne poprawki stylów, dodanie klas dla przycisków */
.view-wrapper {
  display: flex;
...
.print-controls { margin-bottom: 30px; padding: 20px; border: 1px solid #e0e0e0; border-radius: 6px; background-color: #f8f9fa; }
.print-controls h3 { margin-top: 0; }
...
.work-card { border: 1px solid #ccc; padding: 25px; border-radius: 6px; }
.print-title {
  text-align: center;
  margin-top: 0;
  margin-bottom: 30px;
...
.card-item { background-color: #f8f9fa; padding: 15px; border-radius: 6px; border: 1px solid #e7e7e7; font-size: 16px; }
.card-item.full-width { grid-column: 1 / -1; }
.card-item strong { display: block; margin-bottom: 8px; color: #6c757d; font-size: 14px; }
.card-item span { font-weight: 500; }
.info-text { margin: 0; white-space: pre-wrap; line-height: 1.6; }
.error-box, .loading-box { text-align: center; padding: 50px; color: #6c757d; }
.error-box h2 { color: #dc3545; }

/* ZMIANA: Bardziej "agresywne" style dla wydruku */
@media print {
  body, #app, .view-wrapper {
    all: unset !important; /* Usuwa wszystkie style z elementów nadrzędnych */
