...
<template>
  <div class="container">
    ...
    <div v-else>
      <div class="table-container">
        <table>
          ...
          <tbody>
            <tr v-for="praca in prace" :key="praca.id">
              ...
              <td data-label="Pracownicy" class="col-pracownicy">
                {{ praca.pracownicy || '-' }}
              </td>
              <td data-label="Telefon" class="col-telefon">
                {{ praca.numer_tel || '-' }}
              </td>
              <td data-label="Miejscowość">{{ praca.miejscowosc || '-' }}</td>
              <td data-label="Informacje" class="col-informacje">
                {{ praca.informacje || '-' }}
              </td>
              <td data-label="Data Rozp.">
                {{ formatDate(praca.data_rozpoczecia) }}
              </td>
              <td data-label="Data Zakoń.">
                {{ formatDate(praca.data_zakonczenia) }}
              </td>
              <td data-label="Średnica Ø">{{ praca.srednica || '-' }}</td>
              <td data-label="L. statyczne">
                {{ praca.lustro_statyczne || '-' }}
              </td>
              <td data-label="L. dynamiczne">
                {{ praca.lustro_dynamiczne || '-' }}
              </td>
        ...
        <div v-if="!prace.length && !isLoading" class="empty-table-message">
  ...
  <div v-if="showAddModal" class="modal-backdrop">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <button class="close-button" @click="handleCancelAdd">&times;</button>
      </div>
  ...
  <div v-if="showEditModal" class="modal-backdrop">
    <div class="modal-content">
      <div class="modal-header">
        <h3>Edytuj wpis #{{ edytowaneDane.id }}</h3>
        <button class="close-button" @click="handleCancelEdit">&times;</button>
      </div>
