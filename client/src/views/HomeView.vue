      ...
      <table>
        ...
        <tbody>
          <tr v-for="praca in prace" :key="praca.id">
            ...
            <td data-label="Pracownicy">{{ praca.pracownicy }}</td>
            <td data-label="Telefon">{{ praca.numer_tel }}</td>
            <td data-label="Miejscowość">{{ praca.miejscowosc }}</td>
            <td data-label="Informacje">{{ praca.informacje }}</td>
...
<style>
  ...
  @media screen and (max-width: 768px) {
    ...
    .table-container table tr {display:block;margin-bottom:.625em;border:1px solid #ddd;border-radius:6px;padding:10px}
    .table-container table td {display:block;text-align:right;border-bottom:1px dotted #ccc;padding:10px}
    .table-container table td::before {content:attr(data-label);float:left;font-weight:700;text-transform:uppercase}
