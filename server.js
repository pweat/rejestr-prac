...
// ZMIANA: Importujemy "sterownik" do PostgreSQL zamiast sqlite3
...
// ZMIANA: Nowa konfiguracja połączenia z bazą danych
const pool = new Pool({
  user: 'postgres',
  host: 'localhost',
  database: 'rejestr_prac',
  password: 'admin', // <-- WAŻNE: Wpisz tutaj swoje hasło!
  port: 5432,
});
...
const initializeDatabase = async () => {
  try {
    ...
    // ZMIANA: Nowa składnia dla PostgreSQL (SERIAL PRIMARY KEY)
    await client.query(`
...
// === 5. API ENDPOINTS (PRZEPISANE Z UŻYCIEM ASYNC/AWAIT I PG) ===
  ...
  try {
    ...
    if (search) {
      ...
      searchableColumns.forEach(col => {
        whereClauses.push(`${col} ILIKE $${paramIndex++}`); // ILIKE dla wielkości liter
      });
...
app.get('/api/prace/:id', async (req, res) => {
  try {
    ...
    if (result.rows.length === 0) {
      return res.status(404).json({ "error": "Nie znaleziono wpisu o podanym ID." });
    }
